...
def main():
    ...
    ap.add_argument('--runner',required=True); ap.add_argument('--base-module',required=True); ap.add_argument('--v5-module',required=True); ap.add_argument('--data-dir',required=True); ap.add_argument('--output',required=True); ap.add_argument('--mode',choices=['cluster_group_global_label','cluster_group_cluster_label'],required=True)
    a=ap.parse_args(); r=load(Path(a.runner),'cluster_rank_runner_v2'); holder={}; orig_load=r.load_module; orig_fit=r.fit_predict
    ...
    class ClusterXGBRanker:
        ...
        def fit(self,X,y,group=None,verbose=False,**kwargs):
            meta=holder['compact_meta'].loc[X.index].copy(); order=meta.sort_values(['signal_date','cluster_id','ticker']).index; meta2=meta.loc[order]
            ...
            self.inner.fit(X.loc[order],yy,group=groups,verbose=verbose,**kwargs); return self
    ...
    def fit(compact,tail,macro,macro_feats,opp,base,years,n_estimators,models_dir):
        ...
        missing=int(c.cluster_id.isna().sum()); c=c[c.cluster_id.notna()].copy(); c['cluster_id']=c.cluster_id.astype(int)
        if a.mode=='cluster_group_cluster_label': c['target_rank_pct']=c.groupby(['signal_date','cluster_id'])['fwd_ret_monthly'].rank(pct=True,method='average')
        holder['compact_meta']=c[['signal_date','ticker','cluster_id']].copy(); holder['missing_cluster_rows']=missing
    ...
    out=Path(a.output); z=np.load(out/'TITANIUM_CONCENTRATION_FRONTIER_PATHS.npz'); dates=z['dates']; B=z['BASE']; R=z['ROUTER']; res={'mode':a.mode,'compact_maturity':'exit_date_42','missing_cluster_rows_dropped':holder.get('missing_cluster_rows')}; err=0.0
    ...
    (out/'D42_CLUSTER_RANK_V2_SUMMARY.json').write_text(json.dumps(res,indent=2)); print(json.dumps(res,indent=2))

...
m=load('broad_probe_impl','forensics/broad_tail_parity_probe.py')
_orig_broad=m.exact_broad

def exact_broad_complete(base,mats,dates,compact_frames):
    tail,D=_orig_broad(base,mats,dates,compact_frames)
    # Missing deterministic field from the July-27 BASE_FEATS block.
    D['vol_ratio_10_63']=D['vol_10']/D['vol_63'].replace(0,np.nan)
    D['vol_ratio_10_63_rank']=base.cs_pct(D['vol_ratio_10_63'])
    # Rebuild Tail frame after schema completion (safe even if Tail does not consume this field).
    for req in base.TAIL_FEATURES:
        if req not in D:
            raw=req[:-5] if req.endswith('_rank') else req
            D[req]=base.cs_pct(D[raw]) if raw in D else pd.DataFrame(np.nan,index=dates,columns=mats['Close'].columns)
...
def fit_all_rankensemble(base, compact, tail, macro, macro_feats, years, n_estimators):
    ...
    for year in years:
        ...
        q=te[['signal_date','ticker']].copy(); q['compact_raw']=np.mean(raw_list,axis=0); q['compact_rank']=np.mean(rank_list,axis=0)
        ttr=tail[(tail.signal_date<cutoff)&(tail.exit_date_63<cutoff)&tail.y_tailmix.notna()]
        tte=tail[tail.signal_date.dt.year.eq(year)]
        tm=make_pipeline(SimpleImputer(strategy='median'),StandardScaler(),Ridge(alpha=30.0)); tm.fit(ttr[base.TAIL_FEATURES],ttr.y_tailmix)
        tq=tte[['signal_date','ticker']].copy(); tq['tail_raw']=tm.predict(tte[base.TAIL_FEATURES]); preds.append(q.merge(tq,on=['signal_date','ticker'],how='left'))
        mtr=macro[(macro.signal_date<cutoff-pd.Timedelta(days=70))&macro.target_rank.notna()]; mte=macro[macro.signal_date.dt.year.eq(year)]
        if len(mtr)>50 and len(mte):
            mm=make_pipeline(SimpleImputer(strategy='median'),StandardScaler(),Ridge(alpha=50.0)); mm.fit(mtr[macro_feats],mtr.target_rank)
    ...
    p=pd.concat(preds,ignore_index=True); p['tail_rank']=p.groupby('signal_date').tail_raw.rank(pct=True); p['titanium_score_pre_macro']=.70*p.compact_rank+.30*p.tail_rank
    mp=pd.concat(macros,ignore_index=True); mp['macro_z']=mp.groupby('signal_date').macro_raw.transform(lambda x:(x-x.mean())/(x.std(ddof=0)+1e-12))
    tops=[]
    ...
    p['macro_category']=p.ticker.map(base.TICKER_CATEGORY); p=p.merge(pd.DataFrame(tops),on='signal_date',how='left')
    p['macro_bonus']=np.where((p.macro_category==p.top_macro)&(p.macro_gap_z>=.75)&(p.tail_rank>=.80),.15,0.0); p['titanium_score']=p.titanium_score_pre_macro+p.macro_bonus
    return p.sort_values(['signal_date','ticker']),pd.DataFrame(audits),mp

m.exact_broad=exact_broad_complete

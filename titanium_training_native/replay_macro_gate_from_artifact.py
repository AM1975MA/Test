...
def main():
    ...
    mpath=Path(a.membership) if a.membership else root/'BASKET_MEMBERSHIP_500.csv'
    mem=pd.read_csv(mpath);mem.ticker=mem.ticker.astype(str);mem=mem[['basket','ticker']].copy()
    baskets=[tuple(g.sort_values('ticker').ticker.tolist()) for _,g in mem.groupby('basket',sort=True)][:500]
    if len(baskets)!=500 or any(len(x)!=24 for x in baskets):raise RuntimeError(f'expected 500x24 membership, got {len(baskets)} baskets sizes {sorted(set(map(len,baskets)))}')
    ...
    marg=np.asarray(margin,float);valid=np.isfinite(marg);conc={}
    for name,(lo,hi) in periods.items():
        km=np.array([lo<=d.year<=hi for d in pd.DatetimeIndex(cal.signal_date)]);q=marg[:,km];v=np.isfinite(q);vals=q[v]
        conc[name]={'n':int(v.sum()),'pct_100_top1':float(np.mean(vals>=.12)) if len(vals) else None,'margin_mean':float(np.mean(vals)) if len(vals) else None,'margin_median':float(np.median(vals)) if len(vals) else None,'margin_p75':float(np.quantile(vals,.75)) if len(vals) else None,'margin_p90':float(np.quantile(vals,.90)) if len(vals) else None}
    msh=hashlib.sha256(mpath.read_bytes()).hexdigest()
    summary={'variant':'POSTFIT_FROZEN_MACRO_TAIL_GATE_REPLAY','membership_path':str(mpath),'membership_sha256':msh,'membership_tickers':int(mem.ticker.nunique()),'periods':pp,'rmse_pp_5periods':math.sqrt(sum(x['gap_pp']**2 for x in pp.values())/5),'changed_bonus_rows':int(changed.sum()),'old_bonus_rows':int((old_bonus.fillna(0)>0).sum()),'new_bonus_rows':int((pred.macro_bonus>0).sum()),'new_bonus_months':int(z.signal_date.nunique()),'new_bonus_tickers':int(z.ticker.nunique()),'tail_rank_min_bonus':float(z.tail_rank.min()) if len(z) else None,'router_active_months':int(np.asarray(active,bool).sum()),'concentration':conc}

...
FROZEN={'winner_recall_1':0.1383,'winner_recall_3':0.3169,'winner_recall_5':0.4384}
...
def main():
    ...
    x['true_rank']=x.groupby(['signal_date','basket']).fwd_ret_21.rank(ascending=False,method='first')
    winners=x[x.true_rank.eq(1)].copy()
    selected=x[x.pred_rank.eq(1)].copy()
    rows=[]
    periods=[('D1',2017,2019),('D2',2020,2022),('DEV',2017,2022),('HOLD',2023,9999),('FULL',2017,9999)]
    for name,lo,hi in periods:
        w=winners[winners.signal_date.dt.year.between(lo,hi)];q=selected[selected.signal_date.dt.year.between(lo,hi)]
        rows.append({'period':name,'n':len(w),'winner_recall_1':float(w.pred_rank.le(1).mean()),'winner_recall_3':float(w.pred_rank.le(3).mean()),'winner_recall_5':float(w.pred_rank.le(5).mean()),'selected_top1_true_rank_mean':float(q.true_rank.mean()),'selected_top1_fwd21_mean':float(q.fwd_ret_21.mean())})
    ...
    full=out[out.period.eq('FULL')].iloc[0].to_dict();summary={'score_col':score_col,'definition':'recall of the realized basket winner within predicted top-k','periods':rows,'frozen_reference':FROZEN,'full_abs_error':{k:abs(float(full[k])-v) for k,v in FROZEN.items()}}

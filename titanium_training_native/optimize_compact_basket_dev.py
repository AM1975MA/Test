...
def basket_metrics(pred,mem):
    x=pred.merge(mem[['basket','ticker']],on='ticker',how='inner')
    x=x.dropna(subset=['pred','fwd_ret_21']).copy()
    # True basket-relative relevance in (0,1]; higher future return => higher relevance.
    ...
    top=x[x.pred_rank<=3].copy()
    top['disc']=1.0/np.log2(top.pred_rank.to_numpy(float)+1.0)
    dcg=top.groupby(['signal_date','basket']).apply(lambda g: float((g.rel*g.disc).sum()),include_groups=False).rename('dcg3')
    # Ideal DCG@3 from each group's true relevance.
    ...
    true_top['ideal_rank']=true_top.groupby(['signal_date','basket']).cumcount()+1
    true_top['disc']=1.0/np.log2(true_top.ideal_rank.to_numpy(float)+1.0)
    ...
    # Extra diagnostics aligned to Titanium top1/top2 concentration.
    top2=x[x.pred_rank<=2].groupby(['signal_date','basket']).rel.mean().rename('top2_mean_rel')
    m=m.join(top2,on=['signal_date','basket'])
    # Predicted top1 true-top3 hit.
    x['true_rank_desc']=x.groupby(['signal_date','basket'])['fwd_ret_21'].rank(ascending=False,method='min')
    hit=x[x.pred_rank.eq(1)].set_index(['signal_date','basket']).true_rank_desc.le(3).astype(float).rename('top1_hit_true_top3')
    m=m.join(hit,on=['signal_date','basket'])
    return m
...
def objective_factory(panel,features,mem,output,shard):
    def objective(trial):
        ...
        # Titanium is primarily top-1 concentrated, with top-2 as fallback.
        score=.60*nd1+.40*nd3
...
def main():
    ...
    panel=pd.read_parquet(a.panel);panel.signal_date=pd.to_datetime(panel.signal_date);panel.exit_date_21=pd.to_datetime(panel.exit_date_21);panel=panel[(panel.signal_date.dt.year>=2017)&(panel.signal_date.dt.year<=2022)].copy()
    ...
    for t in study.trials:
        ...
        r={'trial':t.number,'score':t.value,**t.params,**t.user_attrs};rows.append(r)

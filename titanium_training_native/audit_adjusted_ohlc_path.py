...
def main():
    ...
    rp=find1(root,'DAILY_OHLCV_ACTIONS_150ETF.parquet');raw=pd.read_parquet(rp);raw.columns=[str(c).strip().lower().replace(' ','_') for c in raw.columns]
    aliases={'adj_close':'adj_close','close':'close','open':'open','high':'high','low':'low'}
    ...
    op=pd.read_parquet(find1(root,'OPEN.parquet'));op.index=pd.to_datetime(op.index).tz_localize(None);op.columns=op.columns.astype(str).str.upper()
    cl=pd.read_parquet(find1(root,'CLOSE.parquet'));cl.index=pd.to_datetime(cl.index).tz_localize(None);cl.columns=cl.columns.astype(str).str.upper()
    ...
    summary={'raw_path':str(rp),'raw_rows':int(len(raw)),'raw_tickers':int(raw.ticker.astype(str).nunique()),'raw_first':str(pd.to_datetime(raw.date).min()),'raw_last':str(pd.to_datetime(raw.date).max()),'raw_columns':list(raw.columns),'open_shape':list(op.shape),'close_shape':list(cl.shape),'checks':checks}

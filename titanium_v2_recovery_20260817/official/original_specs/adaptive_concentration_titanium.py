...
# explicit final policies: mode 0 next-best, mode 1 different-macro
pars=[[0,-9,1],[0,.08,.95],[1,.12,.80],[1,.25,.75],[1,.12,.90],[1,.08,.85]]
names=['TOP1_BASE','ALPHA_M0_TH008_W095','BALANCED_M1_TH012_W080','PROTECTION_M1_TH025_W075','MODERATE_M1_TH012_W090','MODERATE_M1_TH008_W085']
...
res=pd.DataFrame(rows);res.to_csv(OUT/'ADAPTIVE_CONCENTRATION_TITANIUM_SCORECARD.csv',index=False)
# save all explicit paths and compact validation
vr=[];save={'dates':dates.values}
for n,name in enumerate(names):
 save[name]=E[n]
 y=res[res.strategy==name].set_index('period')
 vr.append({'strategy':name,'D1_cagr':y.loc['D1'].cagr,'D1_maxdd':y.loc['D1'].maxdd,'D2_cagr':y.loc['D2'].cagr,'D2_maxdd':y.loc['D2'].maxdd,'HOLD_cagr':y.loc['HOLD'].cagr,'HOLD_maxdd':y.loc['HOLD'].maxdd,'FULL_cagr':y.loc['FULL'].cagr,'FULL_maxdd':y.loc['FULL'].maxdd,'FULL_sharpe':y.loc['FULL'].sharpe,'FULL_calmar':y.loc['FULL'].calmar})
pd.DataFrame(vr).to_csv(OUT/'ADAPTIVE_CONCENTRATION_TITANIUM_FINALISTS.csv',index=False)
np.savez_compressed(OUT/'ADAPTIVE_CONCENTRATION_TITANIUM_PATHS.npz',**save)

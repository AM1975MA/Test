...
def patch_base(path):
    ...
    s=replace_once(s,"gk=.5*np.log(H/L.replace(0,np.nan))**2-(2*np.log(2)-1)*np.log(C/O.replace(0,np.nan))**2\n    compact['gkvol21']=snapshot(np.sqrt(gk.rolling(21,min_periods=21).mean()*252),dates)","gk=(.5*np.log(H/L.replace(0,np.nan))**2-(2*np.log(2)-1)*np.log(C/O.replace(0,np.nan))**2).clip(lower=0)\n    compact['gkvol21']=snapshot(np.sqrt(gk.rolling(21,min_periods=21).mean()*252),dates)",'gk')
    s=replace_once(s,"adv=(C*V).rolling(63,min_periods=42).mean()\n    compact['log_adv63']=snapshot(np.log(adv.where(adv>0)),dates)\n    lv=np.log(V.where(V>0)); compact['volume_surprise21']=snapshot((lv-lv.rolling(21,min_periods=15).mean())/lv.rolling(21,min_periods=15).std(ddof=0).replace(0,np.nan),dates)","dollar=V*C\n    compact['log_adv63']=snapshot(np.log1p(dollar.rolling(63,min_periods=63).mean()),dates)\n    lv=np.log1p(V); compact['volume_surprise21']=snapshot((lv-lv.rolling(21,min_periods=21).mean())/lv.rolling(21,min_periods=21).std().replace(0,np.nan),dates)",'adv-volume')

...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--base',type=Path,required=True); ap.add_argument('--v6',type=Path,required=True); ap.add_argument('--data-dir',type=Path,required=True); ap.add_argument('--output',type=Path,required=True); a=ap.parse_args()
    a.output.mkdir(parents=True,exist_ok=True)
    base=loadmod(a.base,'titanium_base_prodpar'); v6=loadmod(a.v6,'titanium_v6_prodpar')
    mats=v6.load_mats(a.data_dir)
    dates,compact,tail0,D=base.build_features(mats)
    ...
    colsout=['rank','signal_date','ticker','compact_raw','compact_rank','tail_raw','tail_rank','titanium_score_pre_macro','macro_category','top_macro','macro_gap_z','macro_bonus','titanium_score']; colsout=[c for c in colsout if c in chk]
    ...
    report={'checkpoint':str(checkpoint.date()),'top1':str(chk.iloc[0].ticker) if len(chk) else None,'top2':str(chk.iloc[1].ticker) if len(chk)>1 else None,'top1_score':float(chk.iloc[0].titanium_score) if len(chk) else None,'top2_score':float(chk.iloc[1].titanium_score) if len(chk)>1 else None,'margin':float(chk.iloc[0].titanium_score-chk.iloc[1].titanium_score) if len(chk)>1 else None,'known_target':['USO','PALL'],'matches_known':bool(len(chk)>1 and chk.iloc[0].ticker=='USO' and chk.iloc[1].ticker=='PALL'),'n_scored':int(len(chk)),'n_train_months':int(audit.iloc[0].compact_train_dates) if len(audit) else None,'n_train_rows':int(audit.iloc[0].compact_train_rows) if len(audit) else None}
